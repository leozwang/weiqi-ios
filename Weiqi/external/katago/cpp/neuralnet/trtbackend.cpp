...
struct ComputeHandle {
  ...
  ComputeHandle(
    ...
    string plan;
    {
      ...
      for(int i = 0; i < 4; i++) {
        snprintf(deviceIdent + i * 2, sizeof(deviceIdent + i * 2), "%02x", static_cast<unsigned char>(deviceHash[i]));
      }
      ...
      for(int i = 0; i < 6; i++) {
        snprintf(tuneIdent + i * 2, sizeof(tuneIdent + i * 2), "%02x", static_cast<unsigned char>(model->tuneHash[i]));
      }
