...
bool FileUtils::exists(const string& path) {
  try {
    gfs::path gfsPath(gfs::path{path});
...
std::string FileUtils::weaklyCanonical(const std::string& path) {
  gfs::path srcPath(gfs::path{path});
  try {
...
bool FileUtils::isDirectory(const std::string& filename) {
  gfs::path srcPath(gfs::path{filename});
  try {
...
bool FileUtils::tryRename(const std::string& src, const std::string& dst) {
  gfs::path srcPath(gfs::path{src});
  gfs::path dstPath(gfs::path{dst});
  try {
...
vector<string> FileUtils::listFiles(const std::string& dirname) {
  ...
  try {
    for(const gfs::directory_entry& entry: gfs::directory_iterator(gfs::path{dirname})) {
...
void FileUtils::collectFiles(const string& dirname, const std::function<bool(const string&)>& fileFilter, vector<string>& collected)
{
  namespace gfs = ghc::filesystem;
  try {
    for(const gfs::directory_entry& entry: gfs::recursive_directory_iterator(gfs::path{dirname})) {
