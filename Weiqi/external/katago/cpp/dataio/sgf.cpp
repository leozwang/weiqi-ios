  ...
  for(size_t i = 0; i<endHist.moveHistory.size(); i++) {
    ...
    if(gameData != NULL && i >= startTurnIdx) {
      ...
      if(turnAfterStart < gameData->whiteValueTargetsByTurn.size()) {
        ...
        char noResultBuf[32];
        char scoreBuf[32];
        snprintf(winBuf, sizeof(winBuf),"%.2f",targets.win);
        snprintf(lossBuf, sizeof(lossBuf),"%.2f",targets.loss);
        snprintf(noResultBuf, sizeof(noResultBuf),"%.2f",targets.noResult);
        snprintf(scoreBuf, sizeof(scoreBuf),"%.1f",targets.score);
        if(comment.length() > 0)
          comment += " ";
        comment += winBuf;
        comment += " ";
      ...
      if(turnAfterStart < gameData->policyTargetsByTurn.size()) {
        char visitsBuf[32];
        snprintf(visitsBuf, sizeof(visitsBuf),"%d",(int)(gameData->policyTargetsByTurn[turnAfterStart].unreducedNumVisits));
        if(comment.length() > 0)
          comment += " ";
        comment += "v=";
        comment += visitsBuf;
      ...
      if(turnAfterStart < gameData->targetWeightByTurnUnrounded.size()) {
        char weightBuf[32];
        snprintf(weightBuf, sizeof(weightBuf),"%.2f",gameData->targetWeightByTurnUnrounded[turnAfterStart]);
        if(comment.length() > 0)
          comment += " ";
        comment += "weight=";
        comment += weightBuf;
