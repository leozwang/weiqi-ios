...
void Search::printTreeHelper(
  ...
  //Output for this node
  {
    ...
    if(data.childVisits > 0) {
      snprintf(buf, sizeof(buf),"T %6.2fc ",(perspectiveFactor * data.utility * 100.0));
      out << buf;
      snprintf(buf, sizeof(buf),"W %6.2fc ",(perspectiveFactor * data.resultUtility * 100.0));
      out << buf;
      snprintf(buf, sizeof(buf),"S %6.2fc (%+5.1f L %+5.1f) ",
    ...
    // if(hasNNValue) {
    //   snprintf(buf, sizeof(buf),"VW %6.2fc VS %6.2fc ", nnResultValue * 100.0, (nnTotalValue - nnResultValue) * 100.0);
    //   out << buf;
    // }
    // else {
    //   snprintf(buf, sizeof(buf),"VW ---.--c VS ---.--c ");
    //   out << buf;
    // }

    if(depth > 0 && !isnan(data.lcb)) {
      snprintf(buf, sizeof(buf),"LCB %7.2fc ", perspectiveFactor * data.lcb * 100.0);
      out << buf;
    }

    if(!isnan(data.policyPrior)) {
      snprintf(buf, sizeof(buf),"P %5.2f%% ", data.policyPrior * 100.0);
      out << buf;
    }
    if(!isnan(data.weightFactor)) {
      snprintf(buf, sizeof(buf),"WF %5.1f ", data.weightFactor);
      out << buf;
    }
    if(data.playSelectionValue >= 0 && depth > 0) {
      snprintf(buf, sizeof(buf),"PSV %7.0f ", data.playSelectionValue);
      out << buf;
    }

    if(options.printSqs_) {
      snprintf(buf, sizeof(buf),"SMSQ %5.1f USQ %7.5f W %6.2f WSQ %8.2f ", data.scoreMeanSqAvg, data.utilitySqAvg, data.weightSum, data.weightSqSum);
      out << buf;
    }

    if(options.printAvgShorttermError_) {
      std::pair<double,double> wlAndScoreError = getShallowAverageShorttermWLAndScoreError(&node);
      snprintf(buf, sizeof(buf),"STWL %6.2fc STS %5.1f ", wlAndScoreError.first * 100.0, wlAndScoreError.second);
      out << buf;
    }

    // Using child visits here instead of edge visits because edge visits is at least
    // semi-reflected in WF and PSV.
    snprintf(buf, sizeof(buf),"N %7" PRIu64 "  --  ", data.childVisits);
    out << buf;
