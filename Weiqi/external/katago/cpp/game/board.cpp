...
string Location::toStringMach(Loc loc, int x_size)
{
  ...
  if(loc == Board::NULL_LOC)
    return string("null");
  char buf[128];
  snprintf(buf, sizeof(buf),"(%d,%d)",getX(loc,x_size),getY(loc,x_size));
  return string(buf);
}
...
string Location::toString(Loc loc, int x_size, int y_size)
{
  ...
  char buf[128];
  if(x <= 24)
    snprintf(buf, sizeof(buf),"%c%d",xChar[x],y_size-y);
  else
    snprintf(buf, sizeof(buf),"%c%c%d",xChar[x/25-1],xChar[x%25],y_size-y);
  return string(buf);
}
...
void Board::printBoard(ostream& out, const Board& board, Loc markLoc, const vector<Move>* hist) {
  ...
  for(int y = 0; y < board.y_size; y++)
  {
    if(showCoords) {
      char buf[16];
      snprintf(buf, sizeof(buf),"%2d",board.y_size-y);
      out << buf << ' ';
    }
    for(int x = 0; x < board.x_size; x++)
    {
