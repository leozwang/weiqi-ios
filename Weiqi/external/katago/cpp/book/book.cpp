...
void Book::recomputeMultiThreaded(const std::vector<SymBookNode>& newAndChangedNodes, MutexPool& mutexPool, int numThreads) {
  ...
  int64_t visitedDoneValue = nextVisitedDoneValue;
  ...
  int64_t visitedDoneValueForCosts = nextVisitedDoneValue;
...
int64_t Book::exportToHtmlDir(
  ...
  auto doubleToStringFourDigits = [&](double x) {
    std::snprintf(toStringBuf, sizeof(toStringBuf),"%.4f",x);
    return string(toStringBuf);
  };
  auto doubleToStringTwoDigits = [&](double x) {
    std::snprintf(toStringBuf, sizeof(toStringBuf),"%.2f",x);
    return string(toStringBuf);
  };
  auto doubleToStringZeroDigits = [&](double x) {
    std::snprintf(toStringBuf, sizeof(toStringBuf),"%.0f",x);
    return string(toStringBuf);
  };
